...
def forward_to_admin(message):
    ...
    if message.content_type == 'text':
        ...
        try:
            # ارسال پیام به ادمین جهت عدم دریافت در تلگرام شما کامنت شد
            # bot.send_message(ADMIN_ID, f"📩 <b>پیام پشتیبانی جدید!</b>\n\n👤 کد کاربر: <code>{user_id}</code>\n📝 متن پیام:\n{user_text}", parse_mode="HTML", reply_markup=admin_markup)
        ...
        try:
            # ارسال تصویر به ادمین جهت عدم دریافت در تلگرام شما کامنت شد
            # bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"📩 <b>عکس پشتیبانی جدید!</b>\n\n👤 کد کاربر: <code>{user_id}</code>\n📝 توضیحات عکس:\n{caption_text}", parse_mode="HTML", reply_markup=admin_markup)
...
def handle_admin_verification(call):
    ...
    if action == "adm_broadcast":
        ...
        bot.answer_callback_query(call.id)
        return
    elif action == "adm_ban_menu":
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("🚫 مسدود کردن (Ban)", callback_data="adm_ban_user"),
            types.InlineKeyboardButton("🟢 آزاد کردن (Unban)", callback_data="adm_unban_user")
        )
        bot.send_message(ADMIN_ID, "⚙️ گزینه مورد نظر را برای مدیریت مسدودسازی انتخاب کنید:", reply_markup=markup)
        bot.answer_callback_query(call.id)
        return
    elif action == "adm_ban_user":
        ADMIN_STATES[ADMIN_ID] = 'ban_id'
        bot.send_message(ADMIN_ID, "🚫 لطفاً آیدی عددی کاربر مورد نظر را جهت مسدودسازی ارسال کنید:")
        bot.answer_callback_query(call.id)
        return
    elif action == "adm_unban_user":
        ADMIN_STATES[ADMIN_ID] = 'unban_id'
        bot.send_message(ADMIN_ID, "🟢 لطفاً آیدی عددی کاربر مورد نظر را جهت رفع مسدودسازی ارسال کنید:")
        bot.answer_callback_query(call.id)
        return
    elif action == "adm_send_direct":
        ADMIN_STATES[ADMIN_ID] = 'direct_user_id'
        bot.send_message(ADMIN_ID, "📩 لطفاً آیدی عددی کاربر مورد نظر را برای ارسال پیام مستقیم بفرستید:")
        bot.answer_callback_query(call.id)
        return
