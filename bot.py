...
def handle_menu_clicks(call):
    ...
    if action == "menu_get_config":
        ...
        user_balance = db_execute("SELECT balance FROM users WHERE user_id =     user_id INTEGER PRIMARY KEY,
    referred_by INTEGER,
    invite_count INTEGER DEFAULT 0,
    has_received_free INTEGER DEFAULT 0,
    balance INTEGER DEFAULT 0,
...
def handle_menu_clicks(call):
    ...
    if action == "menu_get_config":
        ...
        user_balance = db_execute("SELECT balance FROM users WHERE user_id =     invite_count INTEGER DEFAULT 0,
    has_received_free INTEGER DEFAULT 0,
    balance INTEGER DEFAULT 0,
    is_banned INTEGER DEFAULT 0
)""", commit=True)
...
def handle_menu_clicks(call):
    ...
    if action == "menu_get_config":
        ...
        user_balance = db_execute("SELECT balance FROM users WHERE user_id = ?", (user_id,), fetchone=True)[0    invite_count INTEGER DEFAULT 0,
    has_received_free INTEGER DEFAULT 0,
    balance INTEGER DEFAULT 0,
    is_banned INTEGER DEFAULT 0
)""", commit=True)
